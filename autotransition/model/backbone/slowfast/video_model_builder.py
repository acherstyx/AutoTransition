...
from ..build import BACKBONE_REGISTER as MODEL_REGISTRY
...
class PackPathway(torch.nn.Module):
    ...
    def __init__(self, frame_num_dim, slowfast_alpha):
        super().__init__()
        self.dim = frame_num_dim
    ...
    def forward(self, frames: torch.Tensor):
        ...
        slow_pathway = torch.index_select(
            ...
            self.dim,
            torch.linspace(
                0, frames.shape[self.dim] - 1, frames.shape[self.dim] // self.slowfast_alpha, device=frames.device
            ).long(),
        )
...
def apply_config_to_slowfast(cfg: CfgNode):
    cfg: CfgNode = cfg.BACKBONE.SLOWFAST
    cfg.merge_from_file(cfg.ADDITIONAL_CONFIG)
    return cfg
...
class SlowFast(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        """
        cfg = apply_config_to_slowfast(cfg)
        ...
        # resume backbone from checkpoint
        if cfg.CHECKPOINT is not None:
            # stat_dict = torch.load(cfg.CHECKPOINT, map_location="cpu")
            # self.load_state_dict(stat_dict["model_state"])
            load_checkpoint(cfg.CHECKPOINT, self, data_parallel=False, convert_from_caffe2=True)
        self.pack_pathway = PackPathway(frame_num_dim=2, slowfast_alpha=cfg.SLOWFAST.ALPHA)
    ...
    def _construct_network(self, cfg):
        ...
        self.head = head_helper.ResNetFeatureHead(
            dim_in=[
                width_per_group * 32,
                width_per_group * 32 // cfg.SLOWFAST.BETA_INV,
            ],
        ...
        self.d_feature = width_per_group * 32 + width_per_group * 32 // cfg.SLOWFAST.BETA_INV

    def forward(self, x, bboxes=None):
        # divide input into two pathway to fit the slowfast model
        x = self.pack_pathway(x)
...
class ResNet(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        if self.enable_detection:
            ...
            # )
            raise NotImplementedError("ResNetRoIHead is removed")
        else:
